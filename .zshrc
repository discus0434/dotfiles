...
if [ `uname -m` = "arm64" ]; then
  ...
  __conda_setup="$('~/.anyenv_arm/envs/pyenv/versions/miniforge3-4.10.3-10/bin/conda' 'shell.zsh' 'hook' 2> /dev/null)"
  if [ $? -eq 0 ]; then
      eval "$__conda_setup"
  else
      if [ -f "~/.anyenv_arm/envs/pyenv/versions/miniforge3-4.10.3-10/etc/profile.d/conda.sh" ]; then
          . "~/.anyenv_arm/envs/pyenv/versions/miniforge3-4.10.3-10/etc/profile.d/conda.sh"
      else
          export PATH="~/.anyenv_arm/envs/pyenv/versions/miniforge3-4.10.3-10/bin:$PATH"
      fi
  fi
  unset __conda_setup
else
  # >>> conda initialize >>>
  # !! Contents within this block are managed by 'conda init' !!
  __conda_setup="$('~/.anyenv_intel/envs/pyenv/versions/miniforge3-4.10.1-5/bin/conda' 'shell.zsh' 'hook' 2> /dev/null)"
  if [ $? -eq 0 ]; then
      eval "$__conda_setup"
  fi
  unset __conda_setup
